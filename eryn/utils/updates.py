...
class AdjustGaussianProposalScale(Update):
    def __init__(
        self,
        target_acceptance=0.22,
        supression_factor=0.1,
        ...
        """Adjusted scale for gaussian proposal based on cold chain acceptance rate"""
        self.target_acceptance = target_acceptance
        self.verbose = verbose
        self.max_change, self.supression_factor = max_change, supression_factor
    ...
    def __call__(self, iter, last_sample, sampler, idx=0):
        ...
        if self.time > 0:
            ...
            sampler.moves[idx].scale *= change

            if hasattr(sampler.moves[idx], 'svd'):
                sampler.moves[idx].svd = np.linalg.svd(sampler.moves[idx].scale)

        self.previously_accepted = sampler.backend.accepted[:, 0].copy()
        if self.verbose:
            print(
                self.previously_accepted, "\n", mean_af, change, "\n", sampler.moves[idx].cov
            )
        self.previous_iter = sampler.backend.iteration
        self.time += 1
